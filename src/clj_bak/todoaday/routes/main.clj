(ns todoaday.routes.main
  (:require     [todoaday.config :refer [env]]
                [todoaday.pathom :refer [parser]]
...
(defstate main-handler
          :start
          (let [defaults-config (:ring.middleware/defaults-config env)
                legal-origins   (get env :legal-origins #{"localhost"})]
            (-> not-found-handler
                (wrap-api "/api")
                wrap-transit-params
                wrap-transit-response
